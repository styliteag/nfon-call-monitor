...
import type { Call, PfContact } from "../../../shared/types";
...
function ringingExtensionLabel(call: Call): string {
  const ringingLegs = call.legs.filter((l) => l.status === "ringing");
  if (ringingLegs.length === 0) {
    const first = call.legs[0];
    return first?.extensionName || first?.extension || "";
  }
  if (ringingLegs.length === 1) {
    return ringingLegs[0].extensionName || ringingLegs[0].extension;
  }
  const first = ringingLegs[0].extensionName || ringingLegs[0].extension;
  return `${first} +${ringingLegs.length - 1}`;
}
...
export function ActiveCallBanner({ calls, kopfnummern, kopfnummernMap, pfContacts }: Props) {
  ...
  return createPortal(
    <div className="fixed top-4 left-4 z-50 flex flex-col gap-2 max-w-sm">
      {ringing.map((call) => {
        const extLabel = ringingExtensionLabel(call);
        return (
          <div
            key={call.id}
